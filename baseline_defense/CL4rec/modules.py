...
def compute_info_nce_loss(vec1, vec2, temperature=1.0):
    cos_sim = F.cosine_similarity(vec1, vec2, dim=-1) / temperature
    loss = -torch.log(torch.exp(cos_sim) / torch.exp(cos_sim).sum())
