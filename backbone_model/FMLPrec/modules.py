    ...
    def __init__(self, config):
        ...
        if config["hidden_units"] % config["num_heads"] != 0:
            raise ValueError(
                ...
                "heads (%d)" % (config["hidden_units"], config["num_heads"]))
        self.num_attention_heads = config["num_heads"]
        self.attention_head_size = int(config["hidden_units"] / config["num_heads"])
        self.all_head_size = self.num_attention_heads * self.attention_head_size

        self.query = nn.Linear(config["hidden_units"], self.all_head_size)
        self.key = nn.Linear(config["hidden_units"], self.all_head_size)
        self.value = nn.Linear(config["hidden_units"], self.all_head_size)
...
class FilterLayer(nn.Module):
    def __init__(self, config):
        ...
        self.complex_weight = nn.Parameter(torch.randn(1, (config["maxlen"] + 1)//2 + 1, config["hidden_units"], 2, dtype=torch.float32) * 0.02)
        self.out_dropout = nn.Dropout(config["dropout_rate"])
        self.LayerNorm = LayerNorm(config["hidden_units"], eps=1e-12)
...
class Intermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense_1 = nn.Linear(config["hidden_units"], config["hidden_units"] * 4)
        if isinstance('gelu', str):
            self.intermediate_act_fn = ACT2FN['gelu']
        else:
            self.intermediate_act_fn = 'gelu'

        self.dense_2 = nn.Linear(4 * config["hidden_units"], config["hidden_units"])
        self.LayerNorm = LayerNorm(config["hidden_units"], eps=1e-12)
